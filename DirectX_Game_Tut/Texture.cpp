...
Texture::Texture(const wchar_t* full_path) : Resource(full_path)
{
	...
	if (SUCCEEDED(res))
	{
		...
		if (FAILED(res)) throw std::exception("Error: DirectX::CreateTexture error!");
		...
		if (FAILED(res)) throw std::exception("Error: CreateSamplerState Error");

		res = GraphicsEngine::get()->getRenderSystem()->m_d3d_device->CreateShaderResourceView(m_texture, &desc, &m_shader_res_view);
		if (FAILED(res)) throw std::exception("Error: Create shader resource view!");
